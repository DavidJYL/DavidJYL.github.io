...
function renderTimeline() {
  ...
  // 能量条（波形背景）
  if (typeof audioEnergy !== "undefined" && audioEnergy && audioEnergy.data) {
    var ae = audioEnergy;
    var firstBin = Math.floor(timeline.viewStart * ae.binsPerSec);
    var lastBin = Math.ceil(timeline.viewEnd * ae.binsPerSec);
    if (firstBin < 0) firstBin = 0;
    if (lastBin > ae.data.length) lastBin = ae.data.length;
    if (lastBin - firstBin > 1) {
      var barH = H - timeline.rulerH;
      ctx.beginPath();
      ctx.moveTo(timeToX(firstBin / ae.binsPerSec), H);
      for (var bi = firstBin; bi < lastBin; bi++) {
        var bx = timeToX(bi / ae.binsPerSec);
        var bh = ae.data[bi] * barH * 0.85;
        if (bh < 1) bh = 1;
        ctx.lineTo(bx, H - bh);
      }
      ctx.lineTo(timeToX(lastBin / ae.binsPerSec), H);
      ctx.closePath();
      ctx.fillStyle = "rgba(233,69,96,0.12)";
      ctx.fill();
      // 再描一条细的轮廓线增强对比
      ctx.beginPath();
      for (var bj = firstBin; bj < lastBin; bj++) {
        var bx2 = timeToX(bj / ae.binsPerSec);
        var bh2 = ae.data[bj] * barH * 0.85;
        if (bh2 < 1) bh2 = 1;
        if (bj === firstBin) ctx.moveTo(bx2, H - bh2);
        else ctx.lineTo(bx2, H - bh2);
      }
